...
#Description:   The script builds Calico version v3.3.1 on Linux on IBM Z for Rhel(7.3, 7.4, 7.5), Ubuntu(16.04, 18.04) and SLES(12SP3, 15).
...
git checkout v3.3.1 | tee -a "$CALICOCTL_LOG"
...
git checkout v3.3.1 | tee -a "$TYPHA_LOG"
...
git checkout v3.3.1 | tee -a "$CNI_LOG"
...
git checkout v3.3.1 | tee -a "$NODE_LOG"
